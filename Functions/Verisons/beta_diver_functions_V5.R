...
ADONIS_func <- function(in.matrix="",Distance="",in.Metadata="",prefix="", permutations = 1000){
	...
	if(dim(in.Metadata)[2] == 1){
		...
		in.Metadata <- in.Metadata[rownames(as.matrix(data.dist)),]
	}
...
#	ylab=paste("Comp2", format(100*PCoA$CA$eig[2]/sum(abs(PCoA$CA$eig)), digits=4), "%", sep=" ")
	PCoA<-vegan::capscale(in.matrix ~ 1, dist=distance, metaMDS = F) ### you can choose between the euclidean and the bray (bray curtis) dissimilarity
	Sum_dbrda<-summary(PCoA)
	xlab=paste("Comp1", format( 100*Sum_dbrda$cont$importance["Proportion Explained",1], digits=4), "%", sep=" ")
	ylab=paste("Comp2", format( 100*Sum_dbrda$cont$importance["Proportion Explained",2], digits=4), "%", sep=" ")
