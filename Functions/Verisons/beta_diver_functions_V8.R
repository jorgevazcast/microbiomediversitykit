...
adonis.func<-function(sub.metadata = "" , temp.data.dist="", permutations = 1000){
	...
	for(i in names.metadata ){
		...
		if(any(is.na(sub.metadata[,i])) == TRUE){
			...
			#### Adonis test 
			adononis.res<-adonis(temp.data.dist.noNas~temp.sub.metadta.noNAs,permutations = permutations)
			...
			rm(no.naSamples,temp.matrix,ind.rows,ind.col,temp.data.dist.noNas,temp.sub.metadta.noNAs,adononis.res,p.value.temp,R2.temp,FModel.temp,N )
			next
		...
		#### Adonis test 					
		adononis.res<-adonis(temp.data.dist~temp.sub.metadta.noNAs,permutations = permutations)
...
ADONIS_func <- function(in.matrix="",Distance="",in.Metadata="",prefix="", permutations = 1000){
	...
	res.table.adonis$Variable <- as.character(res.table.adonis$Variable)
...
ggplot_envfit <- function(df_ord,ord,fit,Meadata,alpha_pval,choices=c(1,2)){
	...
	if(dim(df_factors)[1] > 0){  	
		...
		df_factors<-data.frame(x=NULL, y=NULL, var=NULL, p.val=NULL)
	}
	
	if(dim(Meadata)[2] == 1){
		sample_names <- rownames(df_ord)
		colnames <- colnames(Meadata)
		Meadata <- data.frame(Meadata[sample_names,] )
		colnames(Meadata) <- colnames; rownames(Meadata) <- sample_names
		df_ord<-cbind(df_ord,Meadata)
	}else{
		df_ord<-cbind(df_ord,Meadata[rownames(df_ord),])
	}
